...
def main2():
    ...
    One could even do better:
    As all of the apes modulo operations are itself primes, we do not have to keep a dict per item. Only keep
    the modulo of the multiplications of all ape primes. See main3 for improvement.
...
def read_input3(path: str = 'input.txt'):
    ...
    # find the unique modulo operator
    primes = {monkey[2] for monkey in monkeys}
    modulo = reduce(lambda x, y: x*y, primes)
    ...
    for _ in range(10000):
        for idx, monkey in enumerate(inputs):
            ...
            while monkey[0]:
                ...
                # calculate the new worry level (after inspection)
                worry = modulo_evaluator(monkey[1], item, modulo)

                # make the check and get the target
                target = monkey[3][worry % monkey[2] == 0]
