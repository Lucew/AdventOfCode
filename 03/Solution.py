...
def time_the_functions(repetitions: int = 10000):
    main1_timed = timeit.timeit(inspect.getsource(main1), number=repetitions)
    print(main1_timed)
    main2_timed = timeit.timeit(inspect.getsource(main2), number=repetitions)
    unholy_main1_timed = timeit.timeit(inspect.getsource(unholy_main1), number=repetitions)
    unholy_main2_timed = timeit.timeit(inspect.getsource(unholy_main2), number=repetitions)

    print(f'The functions took: '
          f'\n\tmain1:\t\t\t{main1_timed:.2E}'
          f'\n\tmain2:\t\t\t{main2_timed:.2E}'
          f'\n\tunholy_main1:\t{unholy_main1_timed:.2E}'
          f'\n\tunholy_main2:\t{unholy_main2_timed:.2E}')
